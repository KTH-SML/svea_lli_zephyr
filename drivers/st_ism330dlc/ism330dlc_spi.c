...
#define ISM330DLC_SPI_READ		(1 << 7)

LOG_MODULE_DECLARE(ISM330DLC, CONFIG_SENSOR_LOG_LEVEL);

static int ism330dlc_spi_read(const struct device *dev, uint8_t reg_addr, uint8_t *value,
			       uint8_t len)
{
	const struct ism330dlc_config *cfg = dev->config;
	uint8_t buffer_tx[2] = { reg_addr | ISM330DLC_SPI_READ, 0 };
...
static int ism330dlc_spi_write(const struct device *dev, uint8_t reg_addr, uint8_t *value,
				uint8_t len)
{
	const struct ism330dlc_config *cfg = dev->config;
	uint8_t buffer_tx[1] = { reg_addr & ~ISM330DLC_SPI_READ };
...
int ism330dlc_spi_init(const struct device *dev)
{
	...
	data->ctx_spi.read_reg = (stmdev_read_ptr) ism330dlc_spi_read;
	data->ctx_spi.write_reg = (stmdev_write_ptr) ism330dlc_spi_write;
