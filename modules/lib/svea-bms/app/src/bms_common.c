...
void bms_init_config(Bms *bms, int type, float nominal_capacity)
{
    ...
    switch (type) {
        ...
        case CELL_TYPE_GENERIC_LIPO:
            ...
            bms->conf.bal_cell_voltage_min = 4.05F; // balance near top
            bms->conf.cell_dis_voltage = 3.45F;     // soft discharge stop (~10% SOC)
            bms->conf.cell_uv_limit = 3.30F;        // absolute minimum
            bms->conf.cell_uv_reset = 3.45F;        // margin for recovery
