...
static int bms_apply_cell_uvp(Bms *bms)
{
    ...
    for (int i = ARRAY_SIZE(UV_delay_setting) - 1; i > 0; i--) {
        if (bms->conf.cell_uv_delay_s >= UV_delay_setting[i]) {
            protect3.UV_DELAY = i;
            break;
        }
    }
    ...
    bms->conf.cell_uv_delay_s = UV_delay_setting[protect3.UV_DELAY];
...
static int bms_apply_cell_ovp(Bms *bms)
{
    ...
    for (int i = ARRAY_SIZE(OV_delay_setting) - 1; i > 0; i--) {
        if (bms->conf.cell_ov_delay_s >= OV_delay_setting[i]) {
            protect3.OV_DELAY = i;
            break;
        }
    }
    ...
    bms->conf.cell_ov_delay_s = OV_delay_setting[protect3.OV_DELAY];
...
void bms_read_current(Bms *bms)
{
    ...
    if (sys_stat.CC_READY == 1) {
        ...
        if (sys_stat.SCD || sys_stat.OCD) {
            LOG_INF("CC sample during fault: raw=%d, current=%.3f A, SYS_STAT=0x%02x",
                    (int16_t)adc_raw, bms->status.pack_current, sys_stat.byte);
        }
...
void bms_update_error_flags(Bms *bms)
{
    ...
    {
        static int64_t chg_oc_started_ms;
        if (bms->status.pack_current > bms->conf.chg_oc_limit) {
            if (chg_oc_started_ms == 0) {
                chg_oc_started_ms = k_uptime_get();
            }
            if ((k_uptime_get() - chg_oc_started_ms) >= (int64_t)bms->conf.chg_oc_delay_ms) {
                error_flags_temp |= 1U << BMS_ERR_CHG_OVERCURRENT;
            }
        }
        else {
            chg_oc_started_ms = 0; // reset timer when below threshold
        }
    }
    ...
    //     LOG_ERR("No alert from BQ769x0 for >500ms\n");
    //     error_flags_temp |= 1U << BMS_ERR_NO_ALERT;
    // }
    // bms->status.error_flags = error_flags_temp;
    //  Don't bother doing extra check if we got normal errors
    //  if (error_flags_temp != 0)
    //     return;

    // Check for gibberish register values (ADC calibration registers)
    int adc_gain1 = bq769x0_read_byte(BQ769X0_ADCGAIN1);
    int adc_offset_reg = bq769x0_read_byte(BQ769X0_ADCOFFSET);
    int adc_gain2 = bq769x0_read_byte(BQ769X0_ADCGAIN2);

    // If all are zero or negative, that's gibberish (should be nonzero, positive values)
    if (adc_gain1 <= 0 && adc_offset_reg <= 0 && adc_gain2 <= 0) {
        error_flags_temp |= 1U << BMS_ERR_GIBBERISH_REGISTERS;
        LOG_ERR("Detected gibberish ADC calibration registers: gain1=%d, offset=%d, gain2=%d",
                adc_gain1, adc_offset_reg, adc_gain2);
    }
    bms->status.error_flags = error_flags_temp;
    return;
}
        ...
        if (sec_since_interrupt >= sec_since_error) {
            if (sys_stat.DEVICE_XREADY) {
                ...
                if (sec_since_error % 3 == 0) {
                    LOG_INF("Attempting to clear DEVICE_XREADY error");
            ...
            if (sys_stat.OVRD_ALERT) {
                if (sec_since_error % 10 == 0) {
                    LOG_INF("Attempting to clear OVERRIDE_ALERT error");
            ...
            if (sys_stat.UV) {
                ...
                if (bms->status.cell_voltage_min > bms->conf.cell_uv_reset) {
                    LOG_INF("Attempting to clear UNDERVOLTAGE error");
            ...
            if (sys_stat.OV) {
                ...
                if (bms->status.cell_voltage_max < bms->conf.cell_ov_reset) {
                    LOG_INF("Attempting to clear OVERVOLTAGE error");
            ...
            if (sys_stat.SCD) {
                if (sec_since_error % 60 == 0) {
                    LOG_INF("Attempting to clear SHORT_CIRCUIT error");
            ...
            if (sys_stat.OCD) {
                if (sec_since_error % 60 == 0) {
                    LOG_INF("Attempting to clear OVERCURRENT error");
